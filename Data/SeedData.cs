...
public static class SeedData
{
    ...
    private static async Task FixBrokenImageUrlsAsync(IServiceProvider services)
    {
        ...
        // Категорія -> локальне SVG-зображення
        var categoryImages = new Dictionary<string, string[]>
        {
            ["Гірські"]      = new[] { "/images/routes/mountains-1.svg", "/images/routes/mountains-2.svg" },
            ["Пішохідні"]    = new[] { "/images/routes/forest-1.svg" },
            ["Велосипедні"]  = new[] { "/images/routes/bike-1.svg" },
            ["Культурні"]    = new[] { "/images/routes/culture-1.svg" },
            ["Водні"]        = new[] { "/images/routes/water-1.svg" },
        };

        var routes = await db.Routes
            .Include(r => r.Category)
            .Where(r => r.ImageUrl != null && r.ImageUrl.Contains("unsplash.com"))
            .ToListAsync();
        ...
        {
            var catName = r.Category?.Name ?? "Гірські";
            var options = categoryImages.TryGetValue(catName, out var imgs) ? imgs : categoryImages["Гірські"];
            r.ImageUrl = options[rnd.Next(options.Length)];
        }
